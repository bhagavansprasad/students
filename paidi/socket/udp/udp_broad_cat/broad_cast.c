...
int main(int argc, char *argv[])
{
	...
	{                                   /* Broadcast as client */
		...
		numbytes = sendto(sockfd, ctime(&now), 24, 0,(struct sockaddr *)&sendaddr, sizeof sendaddr);
	...
	numbytes = sendto(sockfd, "Hello", 5 , 0,(struct sockaddr *)&sendaddr, sizeof sendaddr);
	...
	for (;;)
	{
		...
		else
		{
			...
			if ((numbytes = recvfrom(sockfd, buf, sizeof buf, 0,(struct sockaddr *)&sendaddr, &addr_len)) > 0)
			{
				time_t now = time(NULL);
