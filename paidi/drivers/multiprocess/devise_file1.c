...
void addatbeg(int param)
{
	node *temp=NULL,*new=NULL,*old=NULL;
	new=kmalloc(sizeof(node),GFP_KERNEL);
	new->val=param;
	new->next=head;
	head=new;
}
int addafter(int loc,int n)
{
	...
	for(i=0;i<loc;i++)
	{
		...
		if(temp==NULL)
		{
			printk("there are less than %d elements\n",loc);
			return;
		}
	}
	new=kmalloc(sizeof(node),GFP_KERNEL);
		...
		unsigned long ioctl_param)
{
	...
	switch(ioctl_num)
	{
		...
		case IOCTL_GET_NTH_BYTE:
			...
			break;
		case CMD_ADD_ATBEG:
			addatbeg(ioctl_param);
			break;
		case CMD_ADDAFTER:
			addafter(2,ioctl_param);
			break;
		case CMD_DISPLAY:
			display(ioctl_param);
			break;
