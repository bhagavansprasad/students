...
int main(int argc, char *argv[])
{
	...
	if(param_cascade_filename==0)
	{
		...
		sprintf(param_cascade_filename,"/home/bhagavan/sources/temp/opencv/data/haarcascade_frontalface_default.xml");
	}
	...
	while(!quit)
	{
		...
		if(param_replayfile_name == 0)
		{
			...
			cascade.detectMultiScale(gray, current_faces, 1.2 ,param_neighbors ,CV_HAAR_SCALE_IMAGE ,Size(param_min_face_size, param_min_face_size), Size(param_max_face_size, param_max_face_size));
		}
		else
		{
		...
		if(numImg % 10 == 0)
		{
			printf("Frame#%d(%.1f sec): doing %d computations (%d * %d). Cross count so far : %d\n", numImg, numImg*1/50.0f, current_faces.size() * (pastFaces.size() - farthest_past_index), current_faces.size(),(pastFaces.size() - farthest_past_index),  crosscount);
		}
		...
		for( unsigned int i = 0; i < current_faces.size(); i++ )
		{
			...
			if(dist_min < FRAME_TO_FRAME_DIST_THRESHOLD)
			{
				...
				if(((closest_y<param_y_line)&&(center.y>=param_y_line)) || ((closest_y>param_y_line)&&(center.y<=param_y_line)) || ((closest_x<param_x_line)&&(center.x>=param_x_line))|| ((closest_x>param_x_line)&&(center.x<=param_x_line)))
				{
