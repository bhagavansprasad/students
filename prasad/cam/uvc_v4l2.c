...
static long uvc_v4l2_do_ioctl(struct file *file, unsigned int cmd, void *arg)
{
	...
	struct uvc_streaming *stream = handle->stream;
	long ret = 0;
	uvc_printk(KERN_INFO,"im in uvc_v4l2_do_ioctl:%lu",VIDIOC_QUERYCAP);
	...
	case VIDIOC_QUERYCAP:
	{
        ...
        uvc_printk(KERN_INFO,"im in uvc_v4l2_do_ioctl cap->driver:%s",cap->driver);
		strlcpy(cap->card, vdev->name, sizeof cap->card);
        uvc_printk(KERN_INFO,"im in uvc_v4l2_do_ioctl vdev->name:%s",vdev->name);
        uvc_printk(KERN_INFO,"im in uvc_v4l2_do_ioctl cap->bus_info  :%s", cap->bus_info);
		usb_make_path(stream->dev->udev,
			      cap->bus_info, sizeof(cap->bus_info));
			
        uvc_printk(KERN_INFO,"im in uvc_v4l2_do_ioctl stream->dev->udev:%s",(char *)stream->dev->udev);
	...
	case VIDIOC_QBUF:
		uvc_printk(KERN_INFO,"im in uvc_v4l2_do_ioctlVIDIOC_QBUF:%lu",VIDIOC_QBUF);
