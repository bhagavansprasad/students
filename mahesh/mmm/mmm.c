...
void *my_thread1(void *vargp)
{
	...
	{
		//	printf("T1 :%p\n", p);
		if(test[j].operation == 1)
		{
			p = amalloc(test[j].size);
			test[j].actualresult=p;
			arr[k]=p;
			k++;
			if(test[j].actualresult == test[j].expected)
			{
			...
			else printf("malloc unsuccessfull in %d\n",test[j].operation);
			
		}
		else if(test[j].operation == 2)
		{
			afree(arr[k-1]);
		}
		else if(test[j].operation == 3)
		{	
			int rv=0;
			p = amalloc(test[j].size);
			test[j].actualresult=p;
			if(test[j].actualresult == test[j].expected)
			{
				...
				else printf("there is no overflow\n");
			}
			else printf("malloc unsuccessfull in %d\n",test[j].operation);

		}
		else if(test[j].operation == 4)
		{
