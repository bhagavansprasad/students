...
main()
{
	...
	int fd[4];
	int spid [4] = {2761,2767,2773,2779};
//	for( i =0;i<4;i++)
//		fd[i] = open("/proc/spid[i]/stat",O_RDONLY);
	fd[0] = open("/proc/2761/stat",O_RDONLY);
	fd[1] = open("/proc/2767/stat",O_RDONLY);
	fd[2] = open("/proc/2773/stat",O_RDONLY);
	fd[3] = open("/proc/2779/stat",O_RDONLY);
	qid = msgget((key_t)90,IPC_CREAT|0666);
	if(qid == -1)
	{
