...
typedef enum FLV_DMX_STATES_
{
	...
	FLV_DMX_STATE_FLV_HDR = 1,   //expectingacquiring header
...
typedef struct audio_video
{
	struct audio_video *prev;
	struct audio_video *next;
	int tag_offset;
	int body_length;
	int body_offset;
	...
	char c_type[124];
	char c_timestamp[124];/* it is never used */

	//Video
	char c_codec_ID[124];
	char c_frame_type[24];

    //Audio
	char c_sound_type[124];
	char c_sound_size[124];
	char c_sound_rate[124];
	char c_sound_format[124];
}tag_data;
