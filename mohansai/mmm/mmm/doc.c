...
int memory_requirements[ROW][COLUM] = 
{
	{100, 2}, 
	{200, 3} 
};
...
static calculate(int someNumber )
{
int rem,hh,mm,ss,ml,i;
...
void mem_rele_status(int size)
{
	...
	{
		 //calculate(temp->front.time_stamp);	
		 printf("node time : curr_time %d:%d, diff :%d\n",
				 (int)temp->front.time_stamp, (int)time(NULL), 
				 (int)(time(NULL) - temp->front.time_stamp));	
		 //printf("size %d time %ld\n\r",temp->front.size,temp->front.time_stamp);
	}
}
...
//	printf("====================================================================\n");
	...
	for(; ;)
	{
		...
		//	arr_status(memory_requirements[i][0]);
			mem_rele_status(memory_requirements[i][0]);
		}
		printf("\n\n");
	}
...
void afree(void *ptr) // freeing the memory
{
	...
	temp = (free_buff_list *)(((char *)ptr) - sizeof(bhead));
	...
	if(hl->tot_available!=0)
	{
		...
		{	
			...
			{	
				...
				temp->front.time_stamp = time(NULL);
