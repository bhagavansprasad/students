...
//		printf("------>the size of each node is %d\n",hv->hs);
	...
	{
		int i,len=0;
            printf("print the num of elemnts in populat function is %d\n", nob);
		for( i = 0; i < nob; i++)
		{
	...
	void *memory_creation()
	{
		...
		for(i=0 ; i < ROWS; i++)
			size = size + ( (mem_req[i][0] + sizeof(struct node) + sizeof(struct header)+sizeof(buffer_tail))*mem_req[i][1])+ ANOTHER*(mem_req[i][0]);
		...
		for(i = 0; i < ROWS; i++ )
		{
			populate_free_buff_list ( p+len, mem_req[i][0], (mem_req[i][1]+ANOTHER));

			len = len + (( mem_req[i][0] + sizeof(struct node) + sizeof(struct header)+sizeof(buffer_tail))* mem_req[i][1]);
		}
...
#if 0
		q4 = amalloc(100);
		printf("--------->the value of q4 amlloc is %p\n",q4);
		sleep(2);
		q5 = amalloc(100);
		printf("--------->the value of q5 amlloc is %p\n",q5);
		sleep(2);
		q6 = amalloc(100);
		printf("--------->the value of q6 amlloc is %p\n",q6);
		sleep(2);
#endif
		display();
		display_data();
