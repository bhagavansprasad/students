...
def init_board():
    ...
    for i in range(1, maxrows+1):
        for j in range(1, maxcols+1):
            ...
            chess_board.update({(i,j):{"color" : cell_color, "coin":"", "pos":(i, j)}})
...
def is_bishap_possible(row, col):
    ...
    if("B1" in coin_2_pos):
        if ("B2" in coin_2_pos["B2"]):
            return False
        elif(chess_board[coin_2_pos["B1"]["color"]] == chess_board[(row, col)]["color"]):
            return False
        else:
            return True
    else:
...
def waysToGiveACheck(board):
    ...
    #print(coin_2_pos["K"])
    #print(chess_board[coin_2_pos["K"]])
    #dum_coin_2_pos()
    #print("P1 :%s" % chess_board[coin_2_pos["P1"]])
    #print("k  :%s" % chess_board[coin_2_pos["k"]])
    promoted_coins = get_promoted_coins_by_pawn(chess_board[coin_2_pos["P1"]])
    promoted_coins = list(promoted_coins)
    #print ("promoted_coins....", promoted_coins)
    nrow, ncol = chess_board[coin_2_pos["P1"]]["pos"]
    #print("k  :%s" % chess_board[coin_2_pos["k"]])
...
def main():
    print (waysToGiveACheck(d1))
    print (waysToGiveACheck(d2))
