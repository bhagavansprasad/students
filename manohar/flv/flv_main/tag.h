...
//	printf("")
	...
	if(frametype == 1)
	{
		...
		if(codecID == 7)
		{
			...
			read(fd,buff,(v_datasize-5));
			write(v_fd,buff,(v_datasize-5));
		}
		else
		{
			read(fd,buff,(v_datasize-1));
			write(v_fd,buff,(v_datasize-1));
		}
		int previoustag=(v_datasize+11);
		body.previoustag=previoustag;
		printf("body previous tag:%x\n",body.previoustag);
		write(v_fd,&body.previoustag,4);
	}
	else
	{
		lseek(fd,(v_datasize-1),SEEK_CUR);
		int previoustag=(v_datasize+11);
		body.previoustag=previoustag;
		printf("body previous tag:%d\n",body.previoustag);
		write(v_fd,&body.previoustag,4);
